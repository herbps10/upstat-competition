...
Weak Invariance does impact the model fit compared to step 1, however negligably: $\Delta$ CFI = .002, $\Delta$ RMSEA = 0.000 , $\Delta$$\chi^2$=187.875, and the $\Delta$ TLI = 0.000.
...
\subsubsection{Metric (Weak) Invariance}
While Weak Invariance does impact the model fit compared to step 1, the change is negligable: $\Delta$ CFI = .002, $\Delta$ RMSEA = .000 , $\Delta$$\chi^2$=168.330, and the $\Delta$ TLI = .000.
...
\subsubsection{Strong Invariance}
Strong Invariance fails to substantively change model fit above and beyond metric invariance: $\Delta$ CFI = .002, $\Delta$ RMSEA = .000 , and the $\Delta$ TLI = .000 and $\Delta$$\chi^2$=187.875. 
<<echo=FALSE>>=
fitMeasures(MI.eng$fit.intercepts, c('chisq','tli', 'cfi','rmsea','aic'))
@
